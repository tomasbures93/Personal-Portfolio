...
public sealed class AuthService : IAuthService
{
    ...
    public AuthService(IWebsiteRepository websiteRepository, IValidate<LoginRequestDto> validate, ILogger<AuthService> logger, IPasswordHasher passwordHasher)
    {
    ...
    public async Task<Result<LoginResponseDto>> LoginAsync(LoginRequestDto loginRequestDto, CancellationToken token)
    {
        ...
        if (config == null)
        {
            ...
            return Result<LoginResponseDto>.Failure(ResultStatus.Error, "Critical Error - Website configuration not found!!");
        }

        var result = _passwordHasher.Verify(config!.PasswordHash, loginRequestDto.Password);

        if(!result.IsSuccess || config.UserName != loginRequestDto.Login)
        {
